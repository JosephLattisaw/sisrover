...
void Server::start_async_accept() {
    ...
    acceptor.async_accept(*temp_socket,
                          std::bind(&Server::handle_accept, this, std::placeholders::_1));
}

void Server::handle_accept(const boost::system::error_code& error) {
    if (!error) {
        std::cout << "server: accepted connection on port " << _port << std::endl;
        reset();                          // resetting socket before setting it
        socket = std::move(temp_socket);  // moving socket so temporary socket can start accepting
                                          // connections again
        start_keepalive();                // starting our keepalive timer
        start_read();                     // starting to read data
    } else {
        std::cerr << "server: attempted to accept connection on port " << _port
                  << " but an error occurred: " << error.message() << std::endl;
        temp_socket.release();  // just in case
    }

    start_async_accept();  // starting to accept connections again now that our connection has been
                           // processed
}
...
void Server::start_read() {
    ...
    boost::asio::async_read(
        *socket, message_buffer, boost::asio::transfer_exactly(sizeof(camsrv::camsrv_message)),
        std::bind(&Server::start_read_header, this, std::placeholders::_1, std::placeholders::_2));
}

void Server::start_read_header(const boost::system::error_code& error,
                               std::size_t bytes_transferred) {
    if (!error) {
        if (message_buffer.size() == sizeof(camsrv::camsrv_message)) {
            const camsrv::camsrv_message* cm =
                boost::asio::buffer_cast<const camsrv::camsrv_message*>(message_buffer.data());
            ...
            switch (cm->command) {
                ...
                default:
                    ...
                    reset();  // disconnecting socket from the server because unknown command was
                              // sent
                    return;
            }
...
void Server::keepalive_async_wait(const boost::system::error_code& error) {
    if (error == boost::asio::error::operation_aborted && socket) {
        ...
        std::cerr << "server: encountered error while processing keep alive: " << error.message()
                  << std::endl;  // unknown situation, try to keep going
}
...
void Server::start_keepalive() {
    ...
    timer.async_wait(std::bind(&Server::keepalive_async_wait, this, std::placeholders::_1));
}
